...
@section('content')
    <div class="w-full max-w-4xl">
        <form id="object-form" action="{{ route('dealer.objects.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-admin-border shadow-admin-card p-6 space-y-8">
            @csrf
    ...
    (function() {
        ...
        var oldProducts = @json(old('product_items', []));
        if (Array.isArray(oldProducts) && oldProducts.length) {
            oldProducts.forEach(function(item) {
                if (!item || item.bitrix_product_id == null || !item.product_name) return;
                addProduct(String(item.bitrix_product_id), String(item.product_name), item.quantity);
            });
        }
