...
@section('heading', 'Мои клиенты')

@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <form action="{{ route('dealer.clients.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
        ...
        <a href="{{ route('dealer.clients.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-admin-accent text-white font-medium hover:bg-admin-accent-hover transition shrink-0">
                        ...
                        <tr class="border-b border-admin-border hover:bg-slate-50/50 transition">
                            ...
                            <td class="py-4 px-6 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('dealer.clients.edit', $client) }}" class="p-2 rounded-lg text-admin-muted hover:bg-admin-accent-soft hover:text-admin-accent transition" title="Редактировать">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    <form action="{{ route('dealer.clients.destroy', $client) }}" method="POST" class="inline" onsubmit="return confirm('Удалить клиента?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-12 px-6 text-center text-admin-muted">
                                Клиентов пока нет. <a href="{{ route('dealer.clients.create') }}" class="text-admin-accent hover:underline">Добавить первого</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
