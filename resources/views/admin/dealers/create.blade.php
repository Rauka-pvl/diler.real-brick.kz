...
@section('title', 'Добавить дилера')
@section('heading', 'Добавить дилера')

@section('content')
    <div class="w-full">
        <form action="{{ route('admin.dealers.store') }}" method="POST" class="bg-white rounded-2xl border border-admin-border shadow-admin-card p-6 space-y-6">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="bin" name="bin" value="{{ old('bin') }}" placeholder="12 цифр"
                           class="w-full px-4 py-3 rounded-xl border border-admin-border focus:border-admin-accent focus:ring-2 focus:ring-admin-accent/20 outline-none">
                </div>
                <div class="sm:col-span-2">
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-admin-fg mb-2">Пароль для входа</label>
                    <div class="relative">
                        <input type="password" id="password" name="password" autocomplete="new-password"
                               ...
                               placeholder="Дилер сменит при первом входе">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" id="legal_address" name="legal_address" value="{{ old('legal_address') }}"
                           class="w-full px-4 py-3 rounded-xl border border-admin-border focus:border-admin-accent focus:ring-2 focus:ring-admin-accent/20 outline-none">
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="2" class="w-full px-4 py-3 rounded-xl border border-admin-border focus:border-admin-accent focus:ring-2 focus:ring-admin-accent/20 outline-none">{{ old('notes') }}</textarea>
                </div>
                <div class="sm:col-span-2 flex items-center">
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }}
                           class="rounded border-admin-border text-admin-accent focus:ring-admin-accent">
                    <label for="is_active" class="ml-2 text-sm text-admin-fg">Активный дилер</label>
                </div>
            </div>
