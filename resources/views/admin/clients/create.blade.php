...
@section('content')
    <div class="w-full">
        <form action="{{ route('admin.clients.store') }}" method="POST" class="bg-white rounded-2xl border border-admin-border shadow-admin-card p-6 space-y-6">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div class="sm:col-span-2">
                    ...
                    <select id="dealer_id" name="dealer_id" required
                        ...
                        <option value="">Выберите диллера</option>
                        @foreach($dealers as $d)
                            <option value="{{ $d->id }}" {{ old('dealer_id') == $d->id ? 'selected' : '' }}>{{ $d->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="type" name="type" required
                        ...
                        <option value="ip" {{ old('type') === 'ip' ? 'selected' : '' }}>ИП</option>
                    </select>
