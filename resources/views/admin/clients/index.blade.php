...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <form action="{{ route('admin.clients.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
            ...
            <select name="type" class="px-4 py-2.5 rounded-xl border border-admin-border focus:border-admin-accent outline-none">
        ...
        <a href="{{ route('admin.clients.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-admin-accent text-white font-medium hover:bg-admin-accent-hover transition shrink-0">
                ...
                <tbody>
                    @forelse($clients as $client)
                        <tr class="border-b border-admin-border hover:bg-slate-50/50 transition">
                            <td class="py-4 px-6">
                                <span class="font-medium text-admin-fg">{{ $client->name }}</span>
                                @if($client->email)
                                    <div class="text-sm text-admin-muted">{{ $client->email }}</div>
                                @endif
                                @if($client->phone)
                                    <div class="text-sm text-admin-muted">{{ $client->phone }}</div>
                                @endif
                            </td>
                            <td class="py-4 px-6 text-admin-fg">
                                @if($client->dealer)
                                    {{ $client->dealer->name }}
                                @else
                                    —
                            ...
                            <td class="py-4 px-6">
                                @if($client->contact_person_name || $client->contact_person_phone)
                                    <span class="text-admin-fg">{{ $client->contact_person_name ?: '—' }}</span>
                                    @if($client->contact_person_position)
                                        <div class="text-sm text-admin-muted">{{ $client->contact_person_position }}</div>
                                    @endif
                                    @if($client->contact_person_phone)
                                        <div class="text-sm text-admin-muted">{{ $client->contact_person_phone }}</div>
                                    @endif
                                @else
                                    —
                                @endif
                            </td>
                            <td class="py-4 px-6 text-admin-fg">{{ $client->city ?? '—' }}</td>
                            <td class="py-4 px-6 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.clients.show', $client) }}" class="p-2 rounded-lg text-admin-muted hover:bg-admin-accent-soft hover:text-admin-accent transition" title="Просмотр">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                        </svg>
                                    </a>
                                    <a href="{{ route('admin.clients.edit', $client) }}" class="p-2 rounded-lg text-admin-muted hover:bg-admin-accent-soft hover:text-admin-accent transition" title="Редактировать">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    <form action="{{ route('admin.clients.destroy', $client) }}" method="POST" class="inline" onsubmit="return confirm('Удалить клиента?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-12 px-6 text-center text-admin-muted">
                                Клиентов пока нет. <a href="{{ route('admin.clients.create') }}" class="text-admin-accent hover:underline">Добавить первого</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
