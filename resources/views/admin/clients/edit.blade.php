...
@section('heading', 'Редактировать клиента')

@section('content')
    <div class="w-full">
        <form action="{{ route('admin.clients.update', $client) }}" method="POST" class="bg-white rounded-2xl border border-admin-border shadow-admin-card p-6 space-y-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div class="sm:col-span-2">
                    ...
                    <select id="dealer_id" name="dealer_id" required
                        ...
                        @foreach($dealers as $d)
                            <option value="{{ $d->id }}" {{ old('dealer_id', $client->dealer_id) == $d->id ? 'selected' : '' }}>{{ $d->name }}</option>
                        @endforeach
                    </select>
                    @error('dealer_id')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div class="sm:col-span-2">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="requisites" name="requisites" rows="4" class="w-full px-4 py-3 rounded-xl border border-admin-border focus:border-admin-accent focus:ring-2 focus:ring-admin-accent/20 outline-none">{{ old('requisites', $client->requisites) }}</textarea>
                </div>
                <div class="sm:col-span-2 border-t border-admin-border pt-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" id="instagram" name="instagram" value="{{ old('instagram', $client->instagram) }}" placeholder="@username"
                                   class="w-full px-4 py-3 rounded-xl border border-admin-border focus:border-admin-accent focus:ring-2 focus:ring-admin-accent/20 outline-none">
                        </div>
                    </div>
                </div>
                <div class="sm:col-span-2 border-t border-admin-border pt-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="contact_person_position" name="contact_person_position" value="{{ old('contact_person_position', $client->contact_person_position) }}"
                                   class="w-full px-4 py-3 rounded-xl border border-admin-border focus:border-admin-accent focus:ring-2 focus:ring-admin-accent/20 outline-none">
                        </div>
                        <div>
                            <label for="contact_person_phone" class="block text-sm font-medium text-admin-fg mb-2">Номер телефона</label>
                            <input type="text" id="contact_person_phone" name="contact_person_phone" value="{{ old('contact_person_phone', $client->contact_person_phone) }}"
                                   class="w-full px-4 py-3 rounded-xl border border-admin-border focus:border-admin-accent focus:ring-2 focus:ring-admin-accent/20 outline-none">
                        </div>
                    </div>
                </div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('admin.clients.index') }}" class="px-5 py-2.5 rounded-xl border border-admin-border text-admin-fg font-medium hover:bg-slate-50 transition">Отмена</a>
            </div>
        </form>
    </div>
@endsection
